...
public class Table {
    ...
    /**
     * Factory method to create a table with predefined rules.
     *
     * @param tableId Table ID (must be between 1 and 10).
     * @param isVip   Defines whether the table is VIP.
     * @return A new Table object.
     * @throws IllegalArgumentException if tableId is out of range.
     */
    public static Table createTable(int tableId, boolean isVip) {
        if (tableId < 1 || tableId > 10) {
            throw new IllegalArgumentException("Table ID must be between 1 and 10.");
        }
        return new Table(tableId, isVip);
    }
